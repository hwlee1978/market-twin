...
const LABELS_BY_LOCALE: Record<string, Parameters<typeof buildReportPdf>[0]["labels"]> = {
  ko: {
    ...
    voicesTitle: "고객 한 마디",
    voicesSub: "페르소나 풀에서 의향대별로 추출한 대표 인용",
    intentLabel: "의향",
  ...
  en: {
    ...
    voicesTitle: "Voices of the Customer",
    voicesSub: "Representative quotes drawn from the persona pool, balanced across intent",
    intentLabel: "Intent",
    generatedBy: "Generated by Market Twin",
    page: "Page",
  },
...
export async function GET(
  ...
  const { data: sim, error: simErr } = await supabase
    .from("simulations")
    .select(
      "id, project_id, persona_count, started_at, completed_at, model_provider, model_version, projects(product_name, currency)",
    )
    .eq("id", simulationId)
  ...
  const projectMeta = Array.isArray(sim.projects)
    ? (sim.projects[0] as { product_name?: string; currency?: string } | undefined)
    : (sim.projects as { product_name?: string; currency?: string } | null);
  const productName = projectMeta?.product_name ?? "";
  const currency = projectMeta?.currency ?? "USD";
