...
== 사용자 추가 컨텍스트 취급 (anti-echo) ==
- 사용자 추가 컨텍스트는 회사가 직접 작성한 자료라 마케팅 표현·과장("혁신적·업계최고·독보적·압도적")이 섞일 수 있습니다. **사실(브랜드명·숫자·제품·채널·소재)은 모두 보존**하되, 검증되지 않은 홍보성 수식어는 사실처럼 옮기지 말고 **중립적으로** 기술하세요.
- 사용자 컨텍스트와 검색 결과가 **검증 가능한 외부 사실**(매출·시장순위·연도 등)에서 충돌하면 **검색 결과를 우선**합니다. (vision·포지셔닝 같은 내부 정보는 사용자 컨텍스트가 우선)
