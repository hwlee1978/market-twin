...
export function ProjectWizard({
  ...
  const toggleCountry = (code: string) =>
    setForm((f) => {
      const dropping = f.countries.includes(code);
      if (!dropping) return { ...f, countries: [...f.countries, code] };
      // Dropping a candidate also drops it from the per-country GTM lists.
      // Otherwise the selection survives invisibly and gets persisted, and
      // the ranker keeps boosting a market that is no longer a candidate.
      const without = (arr: string[]) => arr.filter((c) => c !== code);
      return {
        ...f,
        countries: without(f.countries),
        existingMarkets: without(f.existingMarkets),
        partnerMarkets: without(f.partnerMarkets),
        networkMarkets: without(f.networkMarkets),
      };
    });
      ...
      <div className="card p-8 space-y-6">
        ...
        {stepKey === "countries" && (
          <>
            ...
            {/* Per-country GTM signals. These live here rather than with the
                other brand-strategy hints on the product step: they ask about
                the candidate markets, which are only chosen right above. */}
            <div className="mt-7 pt-5 border-t border-slate-200 space-y-4">
              <div>
                <label className="label">
                  {locale === "ko"
                    ? "이 브랜드가 이미 이점을 가진 시장 (선택)"
                    : "Markets where this brand already has an edge (optional)"}
                </label>
                <p className="text-xs text-slate-500">
                  {locale === "ko"
                    ? "거시 통계로는 안 보이는 브랜드 고유 신호입니다. 해당 국가 점수를 명시적으로 올립니다 — 모르면 비워두세요."
                    : "Brand-specific signals the macro anchors can't see. They explicitly raise that country's score — leave empty if you don't know."}
                </p>
              </div>
            {(
              [
