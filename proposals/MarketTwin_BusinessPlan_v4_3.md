...
| 제출일 | 2026년 5월 (v4.3: 2026-05-17) |
| 변경 사항 (v4.2 → v4.3) | **§4.4 Phase E 실측 결과 honest disclosure** (mean composite 40.4 / 100, 10-product benchmark 2026-05-16), **§5.3 Phase F 공공 API 카탈로그** (35+ API: World Bank/OECD/관세청 UNI-PASS/DART/KOTRA/Hofstede), **§9.2 KPI 정직한 reframe** (80% 단일 KPI → 분기별 단계 목표), **§9.3 Phase F 6주 로드맵 신규** (Q3 2026), **§12.1 실증 가능성** 정직한 baseline 명시 |
...
|---|---|---|---|
| **현재 honest baseline** | 2026-05-17 | **40.4 / 100** | Phase E 6주 closure 측정 완료 |
| Phase F.0 cheap wins | 2026-06 말 | **45-50** | Hofstede + World Bank + Google Trends |
| Phase F.1 brand-level | 2026-08 말 | **55-65** | 관세청 UNI-PASS + DART + KOTRA K-stat |
| Phase F.2 LLM weighting | 2026-09 말 | **60-68** | per-LLM × category trust weighting |
| 챌린지 협력 데이터 통합 (선정 시) | 2026-Q4 | **65-72** | KORIA/KOSME case-by-case 추가 |
| Long-term ceiling | 2027+ | **75-82** | Outcome feedback loop 누적 (사용자 launch 결과) |
...
| 주 | 활동 | 게이트 목표 | API 의존 |
|---|---|---|---|
| F.0-1 | Hofstede 6D fixture + 페르소나 prompt wire | ≥ 45 | Hofstede (정적 JSON) |
| F.0-2 | World Bank Open Data API 통합 | ≥ 47 | World Bank (free) |
| F.0-3 | Google Trends (pytrends) — trendMatch 활성화 | ≥ 48 | pytrends (free) |
| F.1-1 | 관세청 UNI-PASS API 등록·통합 | ≥ 53 | UNI-PASS (한국 정부, free) |
| F.1-2 | DART Open API — 상장 K-export 사업보고서 자동 추출 | ≥ 58 | DART (free) |
| F.1-3 | KOTRA K-stat / 해외시장정보 | ≥ 60 | KOTRA (free) |
| F.2 | Per-LLM × category trust weighting (per_provider_bias_diagnostic 참조) | ≥ 65 | (코드 변경) |
...
| **실증 가능성 (v4.3 정직 reframe)** | **10-product benchmark mean composite 40.4 / 100 (honest baseline, 2026-05-17, 95% CI [26.6, 53.8])** — confident_wrong 67% 감소 + BoJ Relief Sun anchor +44pt dramatic improvement 등 fix-specific 효과 입증. Phase F 6주 closure로 55-65 목표, 챌린지 협력 시 65-72 도달 path 명확 |
