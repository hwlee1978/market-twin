# K-Beauty D2C 해외 출시 결과 vs Market Twin 시뮬레이션 비교 (v3)

> **Status**: Phase A research + Phase B (anchor historization + 3 brand × 2 tier sim) 완료. Hypothesis tier (1 sim × Anthropic-only, $12.48 총) + Decision tier (6 sims × 3 providers, ~$70 총).
>
> **Document version**: v3 (hypothesis + decision tier 비교)
> **Owner**: ㈜미스터에이아이
...
3개 한국 D2C 뷰티 브랜드 해외 출시를 Market Twin sim과 비교, 두 가지 tier에서 측정.

### Hypothesis tier (1 sim × 200 personas × Anthropic-only, ~$4.16/brand)

| Brand | Split | asOfDate | 실제 | Sim Top-1 (voteShare) | Top-1 hit | Actual rank |
|---|---|---|---|---|---|---|
| Anua | HOLDOUT | 2021-12-31 | US | DE (100% STRONG) | ❌ | 6/10 |
| Tirtir | TRUE holdout | 2020-06-30 | **JP** | CN (100% STRONG) | ❌ | 7/10 |
| BoJ | TUNING | 2021-12-31 | US | US (100% STRONG) | ✅ | 1/11 |

→ **1/3 top-1 hit, 모든 sim 100% STRONG** (단일 LLM prior가 잘못된 자신감 표출)

### Decision tier (6 sims × 3 providers anthropic+openai+deepseek, ~$20-25/brand)

| Brand | 실제 | Sim Top-1 (confidence) | Top-1 hit | Hypothesis 대비 변화 |
|---|---|---|---|---|
| Anua | US | **US (STRONG)** | ✅ | **DE → US FLIP** (Anthropic prior 희석) |
| Tirtir | **JP** | KR (WEAK) — data quirk | ❌ | CN → 합의 없음 (WEAK = "모름" 솔직 표출) |
| BoJ | US | US (80% STRONG) | ✅ | 유지, voteShare 약간 감소 (multi-LLM dissent 1표) |

→ **2/3 top-1 hit + WEAK 신호로 어려운 case 정직 표출**

### 핵심 관찰 (가장 중요한 3가지)

1. **Multi-LLM이 single-LLM prior bias 깨뜨림** — Anua **DE → US FLIP**이 가장 명확한 증거. Anthropic Sonnet의 어성초→EU framing이 OpenAI + DeepSeek 6개 sim 합의에서 dissipate. Postmortem 예측 ([Hypothesis-Postmortem.md](./K-Beauty-D2C-Hypothesis-Postmortem.md)) 적중.
2. **Confidence가 정확도와 inversely calibrated** — Hypothesis는 모든 brand에 100% STRONG (자신만만하게 1/3 hit). Decision은 Tirtir에 WEAK (정직하게 "모름" 표출). **WEAK 신호는 "예측 신뢰 말라"는 시스템의 자기 의식** — 잘못된 STRONG 보다 훨씬 가치 있음.
3. **Anchor만으로는 brand-specific 전략 못 잡음** — Tirtir의 일본 면세점 전략 (창업자 인플루언서 네트워크 베팅) 은 Decision tier도 못 잡음. 거시 anchor의 본질적 한계 — 해결책은 brand strategy interview input field 도입.

**투명성 공개**: 3건 중 2건은 holdout. Anua HOLDOUT (fixture 등재, anchor 튜닝 미사용), Tirtir TRUE holdout (fixture 미등재 본 적 없음), BoJ TUNING. Hypothesis 1/3 는 cherry-pick 안 한 정직 baseline, Decision 2/3 는 tier upgrade로 +1 hit lift.
...
### 3.4 Decision tier 재런 비교 (2026-06-03, ~$70 추가)

Hypothesis 결과의 root cause 추정 (자세한 분석은 [Hypothesis-Postmortem.md](./K-Beauty-D2C-Hypothesis-Postmortem.md)) 을 검증하기 위해 동일 input·동일 asOfDate 로 **Decision tier (6 sims × 3 providers)** 재런.

#### Anua decision tier (ensemble `f40e460c`)

- **Result**: US · 100% STRONG · actual rank 1/10 ✅
- **Hypothesis 대비**: DE → **US FLIP** (postmortem HIGH change 예측 적중)
- **Top-3 (meanIntent)**: US=52 / MY=51 / CN=50
- **Cost**: ~$25, 6/6 sims completed
- **해석**: Anthropic 2 + OpenAI 2 + DeepSeek 2 round-robin이 Anthropic의 어성초→EU prior를 dissolve. 6개 sim 모두 US 합의 = strong cross-LLM agreement. 본 brand의 description signal (Olive Young 1위 + 민감성 피부 functional) 이 multi-LLM 시각에서는 US K-beauty 시장과 더 잘 매칭.

#### Tirtir decision tier (ensemble `45a50932`)

- **Result**: KR · 20% WEAK · actual rank 10/11 ❌
- **Hypothesis 대비**: CN → KR (postmortem LOW change 예측은 절반만 적중 — Top-1 자체는 바뀌었지만 JP를 짚지 못함)
- **Top-3 (meanIntent)**: US=48 / VN=47 / TH=46 (JP=44 10위, KR=0 anomaly)
- **Cost**: ~$22, 5/6 sims completed
- **데이터 anomaly**: KR(originatingCountry)이 sim recommendation 으로 나옴. meanIntent=0 즉 persona 데이터 없음 — 1개 sim이 KR을 강하게 옹호한 결과로 추정되지만 다른 sim들의 합의 부재로 voteShare 20%에 그침. 이건 **system이 "자신 없다"고 솔직 신호 보냄**으로 해석.
- **해석**: Anchor 정보가 동일하므로 multi-LLM도 "어디가 winner인지" 합의를 못 냄. JP는 여전히 anchor 신호에서 보이지 않음 → **brand-strategy interview input field 도입 필요성** 의 결정적 근거.

#### BoJ decision tier (ensemble `b7af0ba2`)

- **Result**: US · 80% STRONG · actual rank 1/11 ✅
- **Hypothesis 대비**: 100% → 80% STRONG (postmortem VERY LOW change 예측 적중, 5 sim 중 4 sim 합의)
- **Top-3 (meanIntent)**: US=49 / GB=49 (tied) / VN=47
- **Cost**: ~$20, 5/6 sims completed
- **해석**: Description-level signal (Reddit r/AsianBeauty + Hyram 추종) 이 multi-LLM에서도 일관되게 US를 가리킴. 80% (vs 100%) 는 dissent 1표 — Decision tier가 더 honest한 confidence를 표출.

#### Tier 비교 요약 + 비용·가치 분석

| 항목 | Hypothesis | Decision | Lift |
|---|---|---|---|
| Top-1 hit | 1/3 | 2/3 | +1 brand |
| 평균 cost/brand | $4.16 | ~$22 | 5.3× |
| 평균 wall-clock | ~12분 | ~15분 (parallel) | 1.25× |
| Confidence calibration | 모든 sim 100% STRONG (자신 over) | WEAK 신호로 어려운 case 솔직 | **질적으로 우수** |

**비용 lift / hit lift = ~$70 / +1 hit = $70 per 추가 hit** (3-brand 표본 한정). 통계 의미는 약하지만 **Decision tier의 진짜 가치는 hit count 아니라 confidence calibration** — Tirtir WEAK 신호가 잘못된 STRONG 보다 사용자에게 훨씬 도움 됨.
