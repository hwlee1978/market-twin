...
| **과제 해결방안 (개발계획)** | (1) 판판대로 지원사업 + 수출바우처 + 기업마당 1,385개 공공 데이터를 OpenAI text-embedding-3-small (1,536d)로 벡터화 후 pgvector 코사인 유사도로 Top-K 후보 추출 → Claude Sonnet 4.6 rerank로 적합도·이유 산출. (2) 매칭 결과를 4종 공공데이터 anchor (Hofstede 6-dim · World Bank 거시지표 · KOTRA korCompList · UN Comtrade)로 grounding한 시장분석 리포트 + 5개국어 상품기술서 + 영상·상세페이지 자동 생성. (3) im-not-ai 룰북 자동 후처리로 한국어 AI 글의 번역체·기계적 구조를 사람 글로 정정. |
| **혁신성** | ① **재현성 100% 보장** — 입력 SHA-256 해시 + temperature=0 + 동일 input_hash 캐시로 동일 입력→동일 출력 수학적 보장. ② **공공데이터 grounding 4종 anchor** — Raw LLM이 hallucinate할 수치를 정부·UN·국제기구 데이터로 grounding. ③ **AI 자연도 자동 정정** — 학술 룰북(im-not-ai v2.0, 김도훈 2009·박옥수 2018 등) 기반 40+ AI 패턴 자동 탐지·재작성. ④ **CSV Batch 평가 지원** — 심사기관 일괄 테스트셋(200건/회)을 단일 업로드로 처리, 결과 CSV 다운로드. ⑤ **이미 작동하는 라이브 시스템** — 신청 시점 95% 구현 완료, 심사위원이 즉시 클릭하여 검증 가능. |
| **가치창출 및 시장성** | (가치) 중소·소상공인이 정부 지원사업·수출 바우처를 매칭받는 데 평균 2주 소요되던 작업을 90초로 단축. 마케팅 콘텐츠 외주비 (300만~1000만 원) → 자동 생성 비용 \$0.67. (시장) 한국 중소기업 380만 + 소상공인 700만 (중기부 2024). 챌린지 종료 후 markettwin.ai 본 서비스(B2B SaaS) 및 KOSME·KOTRA·중기부 산하기관 API 연동으로 확장. |
| **과제 제시 (선택 사항)** | — |
...
Stage 4 (Task 2: 마케팅 콘텐츠 생성)
    ...
    · 다국어 상품기술서 5개국 (KR/EN/JP/TW-zh/CN-zh, jsonSchema)
    · 상세페이지 (specs 표 + 시나리오 + FAQ)
    · 홍보영상 5초 (Replicate Kling v1.6 Pro, image-to-video)
    ↓
...
| 다국어 기술서 | Anthropic Claude Sonnet 4.6 | KR·EN·JP·TW-zh·CN-zh 5개국 자연스러운 카피 |
| 윤문 (Humanize) | Anthropic Claude Sonnet 4.6 | im-not-ai 룰북 40+ 패턴 탐지·재작성 + 자가검증 |
| 홍보영상 | Replicate Kling v1.6 Pro | 2025년 출시 i2v 최상 품질, 텍스트 morph 최소 |
...
실제 검증 (화장품/베트남 진출): "UN Comtrade: KR→VN 화장품(HS33) 수출 5억 1천만 달러(2024), YoY +6.2% — 2022년 3억 7천만 달러 대비 2년간 39% 성장" — 추정·창작 없이 실수치 인용.
...
| **Task 2 콘텐츠 생성 시간** | 시장분석 30초 + 다국어 40초 + 윤문 60초 = ~2분 (영상 별도 2-4분) | < 3분 (영상 제외) |
| **Task 2 grounding 인용률** | 4종 anchor 모두 정확히 인용 (Hofstede/WB/KOTRA/Comtrade) | 100% 유지, 추정·창작 0건 |
| **Task 2 윤문 등급** | A-B (변경률 20-30%, S1 잔존 0) | A 등급 80% 이상 |
| **Task 2 비용 (회당)** | ~\$0.67 (영상 포함), ~\$0.17 (영상 제외) | 최적화 시 ~\$0.50 (영상 포함) |
...
| 산출물 | 비용 |
|---|---|
| Task 1 매칭 (임베딩 + LLM rerank) | \$0.14 |
| Task 2 시장분석 리포트 | \$0.04 |
| Task 2 다국어 기술서 (5개국어) | \$0.06 |
| Task 2 Humanize KR (자동 적용) | \$0.07 |
| Task 2 상세페이지 (UI 렌더, LLM 무관) | \$0 |
| Task 2 홍보영상 5초 (Replicate Kling, 요청 시) | \$0.50 |
| **합계 (영상 포함)** | **\$0.67** |
| **합계 (영상 제외)** | **\$0.17** |
