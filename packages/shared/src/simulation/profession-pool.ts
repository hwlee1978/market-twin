...
import { FOOD_DIET_RESTRICTED_CAPS } from "./calibration/profession-caps";
...
const POOLS_KO: Record<string, CategoryPool> = {
  ...
  food: {
    ...
    caps: {
      ...
      "미식 평론가·푸드 디렉터": 1,
      "전문의 (식이 권고)": 2,
      // Diet-restricted caps — see calibration/profession-caps.ts for provenance.
      ...Object.fromEntries(
        FOOD_DIET_RESTRICTED_CAPS.value.archetypesKo.map((a) => [
          a,
          FOOD_DIET_RESTRICTED_CAPS.value.perSimCap,
        ]),
      ),
    },
  },
...
const POOLS_EN: Record<string, CategoryPool> = {
  ...
  food: {
    ...
    caps: {
      ...
      // Mirror of KO caps — see calibration/profession-caps.ts for provenance.
      ...Object.fromEntries(
        FOOD_DIET_RESTRICTED_CAPS.value.archetypesEn.map((a) => [
          a,
          FOOD_DIET_RESTRICTED_CAPS.value.perSimCap,
        ]),
      ),
