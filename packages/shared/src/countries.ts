export const COUNTRIES = [
  { code: "KR", labelKo: "대한민국", labelEn: "South Korea" },
  { code: "JP", labelKo: "일본", labelEn: "Japan" },
  { code: "CN", labelKo: "중국", labelEn: "China" },
  { code: "TW", labelKo: "대만", labelEn: "Taiwan" },
  { code: "US", labelKo: "미국", labelEn: "United States" },
  { code: "CA", labelKo: "캐나다", labelEn: "Canada" },
  { code: "GB", labelKo: "영국", labelEn: "United Kingdom" },
  { code: "DE", labelKo: "독일", labelEn: "Germany" },
  { code: "FR", labelKo: "프랑스", labelEn: "France" },
  { code: "IT", labelKo: "이탈리아", labelEn: "Italy" },
  { code: "ES", labelKo: "스페인", labelEn: "Spain" },
  { code: "NL", labelKo: "네덜란드", labelEn: "Netherlands" },
  { code: "AE", labelKo: "UAE", labelEn: "United Arab Emirates" },
  { code: "SA", labelKo: "사우디아라비아", labelEn: "Saudi Arabia" },
  { code: "SG", labelKo: "싱가포르", labelEn: "Singapore" },
  { code: "MY", labelKo: "말레이시아", labelEn: "Malaysia" },
  { code: "PH", labelKo: "필리핀", labelEn: "Philippines" },
  { code: "AU", labelKo: "호주", labelEn: "Australia" },
  { code: "IN", labelKo: "인도", labelEn: "India" },
  { code: "VN", labelKo: "베트남", labelEn: "Vietnam" },
  { code: "TH", labelKo: "태국", labelEn: "Thailand" },
  { code: "ID", labelKo: "인도네시아", labelEn: "Indonesia" },
  { code: "BR", labelKo: "브라질", labelEn: "Brazil" },
  { code: "MX", labelKo: "멕시코", labelEn: "Mexico" },
];

const COUNTRY_LOOKUP: Record<string, { labelKo: string; labelEn: string }> = Object.fromEntries(
  COUNTRIES.map((c) => [c.code, { labelKo: c.labelKo, labelEn: c.labelEn }]),
);

/**
 * Display a country code as a localized human label (e.g. "KR" → "대한민국" / "South Korea").
 * Falls back to the raw code when the country isn't in our list (defensive — the LLM
 * occasionally returns codes outside the candidate list).
 */
export function getCountryLabel(code: string | null | undefined, locale: string): string {
  if (!code) return "";
  const upper = code.toUpperCase();
  const entry = COUNTRY_LOOKUP[upper];
  if (!entry) return code;
  return locale === "ko" ? entry.labelKo : entry.labelEn;
}
