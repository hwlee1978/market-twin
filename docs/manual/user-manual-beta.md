...
4. 로그인하면 **대시보드**가 첫 화면입니다. 우측 상단 **+ 새 프로젝트**로 시작하세요.

![로그인 / 제품 소개 화면](images/01_login.png)

> **팁**: 처음이라면 입력 부담 없이 **샘플 데모**(하루 3회)로 결과 화면부터 둘러보세요. 검증 횟수는 차감되지 않습니다.
...
결과 화면 **하단**에 *"이 결과가 의사결정에 도움이 되나요?"* 라는 **1~5점 + 한 줄 의견** 설문이 있습니다. **시뮬레이션을 돌릴 때마다 꼭 남겨 주세요.** 30초면 됩니다.
